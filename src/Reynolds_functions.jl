...
function ReynoldsSolve(U, V, X, Y, paramProblem, dΩ, hMin, paramSolver)
  ...
  if isNL
    ...
    showTrace = paramSolver[:showTrace]
  ...
  function NLsolverPhase(nIterations, useBacktracking)
    if useBacktracking
      solver = NLSolver(show_trace=showTrace, store_trace=true, method=:newton, linesearch=BackTracking(), iterations=nIterations)
    else
      solver = NLSolver(show_trace=showTrace, store_trace=true, method=:newton, iterations=nIterations)
    end
